...
namespace Talabat.API.Controllers
{
    public class AccountController : BaseApiController
    {
        ...
        {
            // When the Token is decoded , it's set to property inside the "ControllerBase" class called "User" , the "User" is of type 
            // "ClaimsPrincipal" .. it now contain the claims of the token (incase it's authorized of course) , if the endpoint is not 
            // authorized then "ClaimsPrincipal" will be empty 
            ...
            return Ok(new UserDTO()
            {
                ...
                Email = user.Email,
                Token = await _authService.CreateTokenAsync(user, _userManager) 
                // Here we will call the function of "Refresh Tokens" , next session
            }) ;
        }
        ...
        {
            // var email = User.FindFirstValue(ClaimTypes.Email);
            // var user = await _userManager.FindByEmailAsync(email); // Address is a navigational property so data is not loaded by default 
            // we will make an extension method for the _userManager , in "Extensions" folder

            var user = await _userManager.FindUserWithAddressAsync(User);
            // now the endpoint has a problem , Self Looping with Bi-directional Navigational Property
            // ex: User has address , and address has user , User has address , and address has user  ............................... 
        ...
        [Authorize]
        [HttpGet("usedemail")]             // GET :    baseURL/api/Account/usedemail?email=mahmoud.shoura.dev@gmail.com
        public async Task<ActionResult<bool>> EmailUsed(string email)
        {
            // return await _userManager.Users.FirstOrDefaultAsync(e => e.NormalizedEmail == email.ToUpper()) is not null;
            // or
